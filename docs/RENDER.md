...
- **`prisma generate` during `npm ci`:** the Dockerfile copies `prisma/` before `npm ci` so `postinstall` can find `prisma/schema.prisma`. If you forked an old Dockerfile, compare with the repo’s current `Dockerfile`.
